...
class AppInterface:
    def __init__(self, project_class: Project, data: Data, config):
        ...
        self.label_set_project = Label(
            text="Enter the project number:", bg=BACKGROUND, font=HEAD_TEXT
        )
        self.label_set_project.grid(column=0, row=0, columnspan=4, pady=8, sticky="wn")
        # Buton
        active_project_but = Button(
            text="Project activate",
            width=16,
            height=1,
            bg="white",
            font=BUTTON_TEXT,
            command=self.activate_project,
        )
        active_project_but.grid(column=2, row=1, columnspan=2, sticky="e")

        self.p01_button = Button(
            text=self.config["default_projects"][0],
            width=8,
            height=1,
            bg="white",
            font=BUTTON_TEXT,
            command=self.quick_fn1,
        )
        self.p01_button.grid(column=0, row=2, sticky="e")

        self.p02_button = Button(
            text=self.config["default_projects"][1],
            width=8,
            bg="white",
            height=1,
            font=BUTTON_TEXT,
            command=self.quick_fn2,
        )
        self.p02_button.grid(column=1, row=2, sticky="w")

        self.p03_button = Button(
            text=self.config["default_projects"][2],
            width=8,
            bg="white",
            height=1,
            font=BUTTON_TEXT,
            command=self.quick_fn3,
        )
        self.p03_button.grid(column=0, row=3, sticky="e")

        self.p04_button = Button(
            text=self.config["default_projects"][3],
            width=8,
            bg="white",
            height=1,
            font=BUTTON_TEXT,
            command=self.quick_fn4,
        )
        self.p04_button.grid(column=1, row=3, sticky="w")
        ...
        self.normalize_button = Button(
            ...
            command=self.confirm_normalize,
        )
    ...
    def pause_fn(self):
        ...
        self.label_set_project.config(
            bg=BACKGROUND, text=f"Project '{self.project_class.project_name}' paused."
        )
    ...
    def resume_fn(self):
        ...
        self.label_set_project.config(
            bg=LIGHT_GREEN, text=f"Project '{self.project_class.project_name}' resumed."
        )
    ...
    def quick_fn3(self):
        self.entry.delete(0, END)
        self.entry.insert(0, self.config["default_projects"][2])

    def quick_fn4(self):
        self.entry.delete(0, END)
        self.entry.insert(0, self.config["default_projects"][3])

    def init_col_but(self):
        self.p01_button.config(bg="white")
        self.p02_button.config(bg="white")
        self.p03_button.config(bg="white")
        self.p04_button.config(bg="white")

    def cancel_project(self):
        self.data.remove_last()
        self.clear_field()
        self.label_set_project.config(
            text=f"Canceled: '{self.project_class.project_name}'"
        )
    ...
    def activate_project(self):
        if (
            self.entry.get() == self.config["default_projects"][0]
            or self.entry.get() == self.config["default_projects"][1]
            or self.entry.get() == self.config["default_projects"][2]
            or self.entry.get() == self.config["default_projects"][3]
        ):
            ...
            if self.project_class.project_pause:
                ...
                try:
                    ...
                    self.data.write_data(
                        self.project_class.project_name, self.project_class.project_time
                    )
        ...
        if project == self.config["default_projects"][0]:
            self.p01_button.config(bg=GREEN)
            self.p02_button.config(bg="white")
            self.p03_button.config(bg="white")
            self.p04_button.config(bg="white")
        elif project == self.config["default_projects"][1]:
            self.p01_button.config(bg="white")
            self.p03_button.config(bg="white")
            self.p04_button.config(bg="white")
            self.p02_button.config(bg=GREEN)
        elif project == self.config["default_projects"][2]:
            self.p03_button.config(bg=GREEN)
            self.p01_button.config(bg="white")
            self.p02_button.config(bg="white")
            self.p04_button.config(bg="white")
        elif project == self.config["default_projects"][3]:
            self.p04_button.config(bg=GREEN)
            self.p01_button.config(bg="white")
            self.p02_button.config(bg="white")
            self.p03_button.config(bg="white")
        else:
        ...
        self.clear.config(
            ...
            command=lambda: (
                ...
                if messagebox.askquestion(
                    "Cancel", f"Cancel last {project} project record?"
                )
                == "yes"
                else None
            ),
    ...
    def confirm_normalize(self):
        ...
        response = messagebox.askquestion(
            "Normalize hours",
            f"Normalize month table to {self.config['normalize_hours']} hours per day?",
        )
        if response == "yes":
